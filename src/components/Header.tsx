...
export default function Header({ account, balance }) {
  ...
  const LoginButton = () => {
    ...
    if (account === "") {
      return (
        <Button variant="contained" color="error" onClick={() => dispatch(toggleConnectModal())}>
          Conect
        </Button>
      );
    } else if (chainId && chainId !== fuseFlash.id) {
      return (
        <Button variant="contained" color="error" onClick={() => switchChain({ chainId: fuseFlash.id })}>
          Switch Chain
        </Button>
      );
    } else {
      ...
      <Toolbar>
        <Box sx={{ flexGrow: 1 }}>
          <Grid container columns={6}>
            ...
            <Grid item xs={1}>
              <Grid container flexDirection="row" alignItems="flex-end" justifyContent="right">
                <LoginButton />
              </Grid>
            </Grid>
          </Grid>
